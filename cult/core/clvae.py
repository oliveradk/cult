...
__all__ = ['CLVAE', 'LatentClassifier', 'train_classifier', 'accuracy', 'mnist_test', 'fashion_test',
           'mnist_test_loader', 'fashion_test_loader']
...
def train_classifier(model, train_loader, test_loader, epochs, n_classes=10, hidden_size=50, lr=1e-3):
    ...
    for epoch in range(epochs):
        ...
        for X, y in train_loader:
            ...
            optimizer.step()
            total_loss += loss
        print("epoch", epoch, "acc", accuracy(classifier, model, test_loader))

    return accuracy(classifier, model, test_loader)

# Cell
def accuracy(classifier, model, test_loader):
    total_acc = 0
    size = 0
...
# Cell
mnist_test= CommonMNIST(DATA_PATH, transform=ToTensor(), download=True, train=False)
fashion_test = CommonFashionMNIST(DATA_PATH, transform=ToTensor(), download=True, train=False)

mnist_test_loader = DataLoader(mnist_test, batch_size = len(mnist_test))
fashion_test_loader = DataLoader(fashion_test, batch_size = len(fashion_test))